...
| `GET /intents?view=history&limit=50&cursor=...` | Private history view; addressed and archived records in reverse creation order, same opaque cursor and page maximum. Returns outcome, terminal time, verified commit, and current result refs so a completed request is discoverable without a saved receipt ID. |
