...
| Processing | One fenced active agent claim per repo in the first slice. No automatic takeover of an expired heartbeat. |
...
```json
{
  ...
  "version": 1,
  "messages": [],
  "processing": {"attempts": 0, "claim": null, "retry_after": null, "last_error_code": null, "prepared_result": null},
  "result": null
}
```
...
| open / needs-input | Operator cancels | archived | Reason `cancelled`; if processing, set cancellation requested and wait for safe stop/reconciliation before final transition. |
...
Integrated auth retains the current server's launch-token-to-HttpOnly/SameSite=Strict-cookie exchange and clean-URL redirect, while adding a distinct session-bound CSRF token and write-route checks. The existing shared launch cookie is not already a complete write-auth contract. For HTTPS use Secure cookies; HTTP is permitted only on explicitly configured loopback origins. Write routes reject query-string tokens, and no credential is embedded into widgets or static HTML. The bootstrap response uses no-store and `Referrer-Policy: no-referrer`; tokens must not enter logs. Existing read-only auth behavior remains unchanged unless separately reviewed.
...
3. Acquire the repo-wide cooperative writer guard used by participating agent helpers. Refuse unrelated dirty/staged work rather than stashing or overwriting it. Hold the guard through edit, verification, commit, and acknowledgment. Existing arbitrary shell writers are outside this guard; recheck HEAD and owned file digests before commit, and stop on interference. An operator recovering an abandoned guard must first terminate/confirm death of its worker; heartbeat timeout is not enough.
...
| Cancellation races with commit | Cancel request stops pre-commit work safely; if a commit already exists, reconcile and record the changed result before archiving as completed-then-cancelled. Never report “cancelled without change” when Git contains the change. |
